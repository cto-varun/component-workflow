...
const transformEvent = (context, event, closure) => {
    ...
    if (op && mappings[closure.op]) {
        ...
        switch (mapping.source) {
            case 'context':
                transformedData = jsonata(mapping.expr).evaluate(context);
                if (mapping?.responseMapping) {
                    transformedData.body['responseMapping'] =
...
const substituteWindowVariables = (headers, variablesToSubstitute) => {
    ...
    let { attId, authBearer, token, mechId, profile } = window[
        ...
        Object.keys(variablesToSubstitute).forEach((variable) => {
            if (headersCopy[variable] !== undefined) {
                ...
                variablesToSubstitute[variable].split('.').forEach((v, i) => {
                    value =
                        i === 0
                            ? window[window.sessionStorage?.tabId][v]
                            : (value || {})[v];
                });
                if (value) {
...
const decryptContent = (body, decryptionKey) => {
    let decrypt = CryptoJS.AES.decrypt(
        body,
        CryptoJS.enc.Base64.parse(decryptionKey),
        {
            mode: CryptoJS.mode.ECB,
            padding: CryptoJS.pad.Pkcs7,
        }
...
const processResponse = (responseMapping, request, response, decryption) => {
    const evaluationContext = { request, response };
    const message = {
        isError: false,
    };
    if (responseMapping.success) {
        if (
            responseMapping.success.error &&
            jsonata(responseMapping.success.error.condition).evaluate(
                evaluationContext
            )
        ) {
            message.isError = true;
            message.message = jsonata(
                responseMapping.success.error.messageExpr
            ).evaluate(evaluationContext);
        } else if (responseMapping.success.success) {
            message.message = jsonata(
                responseMapping.success.success.messageExpr
            ).evaluate(evaluationContext);
            if (response?.request?.responseText && decryption) {
                message.successData = decryptContent(
...
plugin.register({
    ...
    helpers: {
        ...
        sendMessage(context, event, data) {
            MessageBus.send(data.event, data.data);
        },
        sendWokflowMessage(context, event, data) {
            const { registrationId, flowId } = context.machine;
            const message = {
                header: {
                    registrationId,
                    workflow: flowId,
            ...
            MessageBus.send(
                'WF.'.concat(flowId).concat('.').concat(data?.event),
                message
            );
        },
        store(context, event, _data) {
            ...
            let transformedData;
            if (_data?.transform) {
                transformedData = jsonata(_data?.transform).evaluate(body);
            }
            data[_data?.key || event.type] = transformedData || { ...body };
            delete data[_data?.key || event.type].type;
        },
        cache(context, event, _data) {
            cache[_data.op](
                _data.key,
                _data?.expr ? jsonata(_data.expr).evaluate(event) : event
            );
        },
        storeResp(context, event, _data) {
            const { data } = context;
            data[_data?.key || event.type] = { ...event.data.data };
        },
        ...
        send(context, event) {
            const { body } = event;
            const { request } = body;
            const { jumpTo } = context;
            const { conditionExpr } = jumpTo[event.type];
            const step = jsonata(conditionExpr).evaluate(request);
            return send({ type: step });
        },
        invokeService(context, event, closure) {
            const { body } = transformEvent(context, event, closure) || event;
            let { datasource } = body;
            const {
                datasources,
                datasourceExpr,
            ...
            if (datasources && datasourceExpr) {
                datasource =
                    datasources[jsonata(datasourceExpr).evaluate(request)];
            }

            if (requestMapping) {
                const mappedData = jsonata(requestMapping).evaluate(request);
            ...
            if (requestParams) {
                if (params) {
                    ...
                    url = url
                        .concat('?')
                        .concat(new URLSearchParams(filteredParams).toString());
                } else {
                ...
                instance.interceptors.response.use(
                    ...
                    (error) => {
                        if (decryption && error?.response?.data) {
                            try {
                                error.response.data = decryptContent(
                                    error.response.data,
                                    decryption.key
                                );
                        ...
                        if (responseMapping) {
                            ...
                            if (error.response && responseMapping.error) {
                                message.message = jsonata(
                                    responseMapping.error.messageExpr
            ...
            if (method === 'get' || method === 'delete') {
                let content = {
                    headers: substituteWindowVariables(
                        headers,
                        windowVariables
                    ),
                };
